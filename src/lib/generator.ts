...
function crossHatchRegion(paths: string[], spacing: number, angle1: number, angle2: number, sw = 0.4) {
  let o = "";
  ...
  for (let i = -diag; i <= diag; i += spacing) {
    o += L(i * cos1 - diag * sin1, i * sin1 + diag * cos1, i * cos1 + diag * sin1, i * sin1 - diag * cos1, sw * 0.5);
    o += L(i * cos2 - diag * sin2, i * sin2 + diag * cos2, i * cos2 + diag * sin2, i * sin2 - diag * cos2, sw * 0.3);
  }
  return o;
}

function muscleContour(cx: number, cy: number, w: number, h: number, groups: number, r: () => number, sw = 0.5) {
  let o = "";
  for (let g = 0; g < groups; g++) {
    const gx = cx + rr(r, -w * 0.3, w * 0.3);
    const gy = cy + rr(r, -h * 0.3, h * 0.3);
    const gw = w * rr(r, 0.15, 0.35);
    const gh = h * rr(r, 0.1, 0.25);
    const angle = rr(r, -0.3, 0.3);
    const curves = ri(r, 3, 6);
    for (let c = 0; c < curves; c++) {
      const t = (c + 1) / (curves + 1);
      const cw = gw * (1 - t * 0.3);
      const cy2 = gy - gh / 2 + t * gh;
      const curve = gh * 0.3 * Math.sin(t * Math.PI);
      o += P(`M${gx - cw} ${cy2} Q${gx} ${cy2 + curve} ${gx + cw} ${cy2}`, sw);
    }
  }
  return o;
}

function furDirection(cx: number, cy: number, w: number, h: number, count: number, baseAngle: number, r: () => number, sw = 0.3) {
  let o = "";
  for (let i = 0; i < count; i++) {
    const sx = cx + rr(r, -w / 2, w / 2);
    const sy = cy + rr(r, -h / 2, h / 2);
    const len = rr(r, 6, 18);
    const a = baseAngle + rr(r, -0.5, 0.5);
    const curl = rr(r, -4, 4);
    const mx = sx + Math.cos(a) * len * 0.5 + curl;
    const my = sy + Math.sin(a) * len * 0.5 + curl * 0.3;
    const ex = sx + Math.cos(a) * len;
    const ey = sy + Math.sin(a) * len;
    o += P(`M${sx} ${sy} Q${mx} ${my} ${ex} ${ey}`, sw);
  }
  return o;
}
...
function detailedEye(cx: number, cy: number, sc: number, facing: number, r: () => number): string {
  ...
  o += P(`M-16 0 C-16 -9 -10 -15 0 -15 C10 -15 16 -9 16 0 C16 9 10 14 0 14 C-10 14 -16 9 -16 0Z`, 2.5);
  o += P(`M-14 0 C-14 -8 -9 -13 0 -13 C9 -13 14 -8 14 0 C14 8 9 12 0 12 C-9 12 -14 8 -14 0Z`, 0.8);

  for (let i = 0; i < ri(r, 10, 16); i++) {
    const la = (i / 15) * Math.PI * 2;
    const innerR = 13;
    const outerR = rr(r, 18, 24);
    o += P(`M${Math.cos(la) * innerR} ${Math.sin(la) * innerR} Q${Math.cos(la) * (innerR + outerR) * 0.5 + rr(r, -2, 2)} ${Math.sin(la) * (innerR + outerR) * 0.5 + rr(r, -2, 2)} ${Math.cos(la) * outerR} ${Math.sin(la) * outerR}`, 0.4);
  }

  for (let b = 0; b < ri(r, 6, 10); b++) {
    const ba = -Math.PI * 0.7 + (b / 9) * Math.PI * 0.5;
    o += P(`M${Math.cos(ba) * 15} ${Math.sin(ba) * 14} L${Math.cos(ba) * rr(r, 20, 26)} ${Math.sin(ba) * rr(r, 18, 24)}`, 0.5);
  }

  for (let b = 0; b < ri(r, 4, 7); b++) {
    const ba = Math.PI * 0.25 + (b / 6) * Math.PI * 0.45;
    o += P(`M${Math.cos(ba) * 15} ${Math.sin(ba) * 14} L${Math.cos(ba) * rr(r, 19, 23)} ${Math.sin(ba) * rr(r, 17, 21)}`, 0.4);
  }

  o += C2(0, 1, 7.5, 2.5);
  o += stippleCircle(0, 1, 6.5, 4, r);
  ...
  o += P(`M-6 -6 C-4 -9 4 -9 6 -6`, 0.6);
  o += P(`M-5 -8 C-3 -10 3 -10 5 -8`, 0.4);
  o += P(`M-4 -9 C-2 -11 2 -11 4 -9`, 0.3);
...
function detailedNose(cx: number, cy: number, sc: number, r: () => number): string {
  ...
  o += P(`M-22 0 C-24 -10 -20 -20 -12 -25 C-6 -27 6 -27 12 -25 C20 -20 24 -10 22 0 C20 10 14 16 0 18 C-14 16 -20 10 -22 0Z`, 2.8);
  o += P(`M-20 0 C-22 -9 -18 -18 -11 -23 C-5 -25 5 -25 11 -23 C18 -18 22 -9 20 0 C18 9 12 14 0 16 C-12 14 -18 9 -20 0Z`, 1);

  o += stipple(0, -8, 36, 28, 5, r);

  o += P(`M-5 2 C-7 0 -9 -3 -7 -5 C-5 -7 0 -6 0 -3 C0 -6 5 -7 7 -5 C9 -3 7 0 5 2 C3 4 -3 4 -5 2Z`, 2);
  o += stippleCircle(-2, -2, 4, 3, r);
  o += stippleCircle(2, -2, 4, 3, r);

  o += L(-4, -1, 4, -1, 0.7);
  o += P(`M-10 -7 C-5 -10 5 -10 10 -7`, 0.7);
  o += P(`M-14 -12 C-7 -16 7 -16 14 -12`, 0.5);

  o += P(`M0 18 C-3 22 -5 27 -4 32`, 1.5);
  o += P(`M0 18 C3 22 5 27 4 32`, 1.5);
  o += P(`M0 18 C-2 23 -3 28 -2 33`, 0.6);
  o += P(`M0 18 C2 23 3 28 2 33`, 0.6);

  for (let w = 0; w < ri(r, 5, 8); w++) {
    const wy = 18 + w * 3.5;
    const ww = 9 + w * 3.5;
    o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 1, 4)} ${ww} ${wy}`, 0.4);
    o += P(`M${-ww + 2} ${wy + 1} Q0 ${wy + rr(r, 1, 3)} ${ww - 2} ${wy + 1}`, 0.25);
  }
...
function pitbullHead(cx: number, cy: number, sc: number, facing: number, expression: number, earType: number, r: () => number): string {
  ...
  o += P(`M${-hw} 2 C${-hw - 12} ${-hh * 0.4} ${-hw - 8} ${-hh - 5} ${-hw * 0.65} ${-hh - 12} C${-hw * 0.25} ${-hh - 18} ${hw * 0.25} ${-hh - 18} ${hw * 0.65} ${-hh - 12} C${hw + 8} ${-hh - 5} ${hw + 12} ${-hh * 0.4} ${hw} 2 C${hw + 4} ${hh * 0.4} ${hw - 4} ${hh * 0.75} ${hw * 0.72} ${hh + 6} C${hw * 0.35} ${hh + 12} ${-hw * 0.35} ${hh + 12} ${-hw * 0.72} ${hh + 6} C${-hw + 4} ${hh * 0.75} ${-hw - 4} ${hh * 0.4} ${-hw} 2Z`, 3.2);
  o += P(`M${-hw + 4} 2 C${-hw - 8} ${-hh * 0.38} ${-hw - 5} ${-hh - 2} ${-hw * 0.62} ${-hh - 9} C${-hw * 0.22} ${-hh - 15} ${hw * 0.22} ${-hh - 15} ${hw * 0.62} ${-hh - 9} C${hw + 5} ${-hh - 2} ${hw + 8} ${-hh * 0.38} ${hw - 4} 2 C${hw - 2} ${hh * 0.37} ${hw - 6} ${hh * 0.73} ${hw * 0.7} ${hh + 4} C${hw * 0.33} ${hh + 10} ${-hw * 0.33} ${hh + 10} ${-hw * 0.7} ${hh + 4} C${-hw + 6} ${hh * 0.73} ${-hw + 2} ${hh * 0.37} ${-hw + 4} 2Z`, 0.8);

  o += furDirection(0, -hh * 0.1, hw * 1.5, hh * 1.5, 100, -1.4, r, 0.35);
  o += furDirection(0, -hh * 0.3, hw * 1.0, hh * 0.5, 50, -1.55, r, 0.25);
  o += muscleContour(0, -10, hw * 1.2, hh * 1.0, 6, r, 0.5);

  const hwr = hw * 0.4;
  o += P(`M${-hwr} ${-hh * 0.92} C${-hwr * 1.1} ${-hh * 1.15} ${-hwr * 0.7} ${-hh * 1.32} 0 ${-hh * 1.28} C${hwr * 0.7} ${-hh * 1.32} ${hwr * 1.1} ${-hh * 1.15} ${hwr} ${-hh * 0.92}`, 2.2);
  o += P(`M${-hwr * 0.9} ${-hh * 0.9} C${-hwr * 0.95} ${-hh * 1.08} ${-hwr * 0.55} ${-hh * 1.22} 0 ${-hh * 1.2} C${hwr * 0.55} ${-hh * 1.22} ${hwr * 0.95} ${-hh * 1.08} ${hwr * 0.9} ${-hh * 0.9}`, 0.7);
  for (let w = 0; w < ri(r, 3, 6); w++) {
    const wy = -hh * 1.0 + w * 3.5;
    const ww = hw * 0.22 + w * 3;
    o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.4);
  }

  o += P(`M${-hw * 0.42} ${-hh - 14} C${-hw * 0.55} ${-hh - 30} ${-hw * 0.3} ${-hh - 48} ${-hw * 0.08} ${-hh - 45} C${hw * 0.02} ${-hh - 42} ${-hw * 0.05} ${-hh - 20} ${-hw * 0.12} ${-hh - 6}`, 2.8);
  o += P(`M${hw * 0.42} ${-hh - 14} C${hw * 0.55} ${-hh - 30} ${hw * 0.3} ${-hh - 48} ${hw * 0.08} ${-hh - 45} C${-hw * 0.02} ${-hh - 42} ${hw * 0.05} ${-hh - 20} ${hw * 0.12} ${-hh - 6}`, 2.8);
  o += P(`M${-hw * 0.4} ${-hh - 12} C${-hw * 0.5} ${-hh - 27} ${-hw * 0.27} ${-hh - 43} ${-hw * 0.1} ${-hh - 40}`, 0.7);
  o += P(`M${hw * 0.4} ${-hh - 12} C${hw * 0.5} ${-hh - 27} ${hw * 0.27} ${-hh - 43} ${hw * 0.1} ${-hh - 40}`, 0.7);
  for (let w = 0; w < ri(r, 3, 5); w++) {
    const wa = -Math.PI * 0.6 + w * 0.2;
    o += P(`M${-hw * 0.4 + Math.cos(wa) * 5} ${-hh - 14 + Math.sin(wa) * 5} Q${-hw * 0.35 + rr(r, -3, 3)} ${-hh - 20 - w * 4} ${-hw * 0.2 + rr(r, -2, 2)} ${-hh - 16 - w * 3}`, 0.4);
    o += P(`M${hw * 0.4 - Math.cos(wa) * 5} ${-hh - 14 + Math.sin(wa) * 5} Q${hw * 0.35 + rr(r, -3, 3)} ${-hh - 20 - w * 4} ${hw * 0.2 + rr(r, -2, 2)} ${-hh - 16 - w * 3}`, 0.4);
  }

  if (earType === 0) {
    o += P(`M${-hw * 0.62} ${-hh * 0.78} C${-hw * 0.95} ${-hh * 1.5} ${-hw * 0.6} ${-hh * 1.9} ${-hw * 0.32} ${-hh * 1.4} C${-hw * 0.22} ${-hh * 1.1} ${-hw * 0.32} ${-hh * 0.75} ${-hw * 0.48} ${-hh * 0.7}`, 2.8);
    o += P(`M${-hw * 0.6} ${-hh * 0.8} C${-hw * 0.88} ${-hh * 1.4} ${-hw * 0.55} ${-hh * 1.78} ${-hw * 0.34} ${-hh * 1.32}`, 0.7);
    o += furDirection(-hw * 0.5, -hh * 1.2, 22, 28, 18, -1.0, r, 0.35);
    o += stipple(-hw * 0.5, -hh * 1.2, 14, 18, 7, r);
    o += P(`M${hw * 0.62} ${-hh * 0.78} C${hw * 0.95} ${-hh * 1.5} ${hw * 0.6} ${-hh * 1.9} ${hw * 0.32} ${-hh * 1.4} C${hw * 0.22} ${-hh * 1.1} ${hw * 0.32} ${-hh * 0.75} ${hw * 0.48} ${-hh * 0.7}`, 2.8);
    o += P(`M${hw * 0.6} ${-hh * 0.8} C${hw * 0.88} ${-hh * 1.4} ${hw * 0.55} ${-hh * 1.78} ${hw * 0.34} ${-hh * 1.32}`, 0.7);
    o += furDirection(hw * 0.5, -hh * 1.2, 22, 28, 18, -2.1, r, 0.35);
    o += stipple(hw * 0.5, -hh * 1.2, 14, 18, 7, r);
  } else if (earType === 1) {
    o += P(`M${-hw * 0.58} ${-hh * 0.82} C${-hw * 0.7} ${-hh * 1.18} ${-hw * 0.6} ${-hh * 1.38} ${-hw * 0.38} ${-hh * 1.18} C${-hw * 0.26} ${-hh * 0.98} ${-hw * 0.32} ${-hh * 0.82} ${-hw * 0.44} ${-hh * 0.76}`, 3.2);
    o += P(`M${-hw * 0.56} ${-hh * 0.84} C${-hw * 0.66} ${-hh * 1.1} ${-hw * 0.57} ${-hh * 1.3} ${-hw * 0.4} ${-hh * 1.12}`, 0.7);
    o += furDirection(-hw * 0.48, -hh * 1.05, 16, 20, 12, -1.4, r, 0.3);
    o += P(`M${hw * 0.58} ${-hh * 0.82} C${hw * 0.7} ${-hh * 1.18} ${hw * 0.6} ${-hh * 1.38} ${hw * 0.38} ${-hh * 1.18} C${hw * 0.26} ${-hh * 0.98} ${hw * 0.32} ${-hh * 0.82} ${hw * 0.44} ${-hh * 0.76}`, 3.2);
    o += P(`M${hw * 0.56} ${-hh * 0.84} C${hw * 0.66} ${-hh * 1.1} ${hw * 0.57} ${-hh * 1.3} ${hw * 0.4} ${-hh * 1.12}`, 0.7);
    o += furDirection(hw * 0.48, -hh * 1.05, 16, 20, 12, -1.75, r, 0.3);
  } else {
    o += P(`M${-hw * 0.58} ${-hh * 0.78} C${-hw * 0.8} ${-hh * 1.35} ${-hw * 0.72} ${-hh * 1.78} ${-hw * 0.42} ${-hh * 1.58} C${-hw * 0.22} ${-hh * 1.38} ${-hw * 0.26} ${-hh * 0.92} ${-hw * 0.42} ${-hh * 0.76}`, 2.8);
    o += P(`M${-hw * 0.56} ${-hh * 0.8} C${-hw * 0.76} ${-hh * 1.28} ${-hw * 0.68} ${-hh * 1.68} ${-hw * 0.44} ${-hh * 1.5}`, 0.7);
    o += furDirection(-hw * 0.5, -hh * 1.25, 20, 30, 16, -1.0, r, 0.35);
    o += stipple(-hw * 0.5, -hh * 1.25, 16, 24, 7, r);
    o += P(`M${hw * 0.58} ${-hh * 0.78} C${hw * 0.8} ${-hh * 1.35} ${hw * 0.72} ${-hh * 1.78} ${hw * 0.42} ${-hh * 1.58} C${hw * 0.22} ${-hh * 1.38} ${hw * 0.26} ${-hh * 0.92} ${hw * 0.42} ${-hh * 0.76}`, 2.8);
    o += P(`M${hw * 0.56} ${-hh * 0.8} C${hw * 0.76} ${-hh * 1.28} ${hw * 0.68} ${-hh * 1.68} ${hw * 0.44} ${-hh * 1.5}`, 0.7);
    o += furDirection(hw * 0.5, -hh * 1.25, 20, 30, 16, -2.1, r, 0.35);
    o += stipple(hw * 0.5, -hh * 1.25, 16, 24, 7, r);
  }

  o += detailedEye(-24, -12, 1.05, 1, r);
  o += detailedEye(24, -12, 1.05, -1, r);

  const browW = rr(r, 18, 24);
  if (expression === 0) {
    o += P(`M${-browW} ${-32} C${-browW + 7} ${-36} ${-7} ${-38} 0 ${-36}`, 2.2);
    o += P(`M${browW} ${-32} C${browW - 7} ${-36} ${7} ${-38} 0 ${-36}`, 2.2);
    o += P(`M${-browW + 3} ${-31} C${-browW + 6} ${-34} ${-5} ${-35} 0 ${-34}`, 0.6);
    o += P(`M${browW - 3} ${-31} C${browW - 6} ${-34} ${5} ${-35} 0 ${-34}`, 0.6);
  } else if (expression === 1) {
    o += P(`M${-browW} ${-28} C${-browW + 7} ${-33} ${-7} ${-34} 0 ${-32}`, 2);
    o += P(`M${browW} ${-28} C${browW - 7} ${-33} ${7} ${-34} 0 ${-32}`, 2);
  } else {
    o += P(`M${-browW} ${-34} Q${-browW + 7} ${-28} 0 ${-30}`, 2.2);
    o += P(`M${browW} ${-34} Q${browW - 7} ${-28} 0 ${-30}`, 2.2);
    o += P(`M${-browW + 3} ${-33} Q${-browW + 6} ${-29} 0 ${-30}`, 0.5);
    o += P(`M${browW - 3} ${-33} Q${browW - 6} ${-29} 0 ${-30}`, 0.5);
  }

  o += detailedNose(0, 18, 1.15, r);

  o += P(`M-24 40 C-18 50 18 50 24 40`, 2.8);
  o += P(`M-22 42 C-16 50 16 50 22 42`, 0.8);

  for (let l = 0; l < ri(r, 6, 10); l++) {
    const lx = -20 + l * 4.5;
    const ly = 41 + Math.sin(l * 0.7) * 1.5;
    o += L(lx, ly, lx, ly + 5, 0.4);
  }

  const mouthOpen = ri(r, 0, 2);
  if (mouthOpen === 0) {
    o += P(`M-30 44 C-14 56 14 56 30 44`, 2.8);
    o += P(`M-27 46 C-14 54 14 54 27 46`, 0.8);
    o += P(`M-24 49 C-16 58 16 58 24 49`, 1.2);
    o += P(`M-20 51 Q0 56 20 51`, 0.5);
    for (let t = -3; t <= 3; t++) {
      const tx = t * 5.5;
      const ty = 45 + Math.abs(t) * 0.5;
      o += L(tx, ty, tx, ty - 6, t === 0 ? 1.5 : 1);
      o += L(tx - 1.5, ty - 6, tx + 1.5, ty - 6, 0.5);
    }
    for (let t = -3; t <= 3; t++) {
      const tx = t * 5.5;
      const ty = 52 + Math.abs(t) * 0.5;
      o += L(tx, ty, tx, ty + 5, t === 0 ? 1.5 : 1);
      o += L(tx - 1.5, ty + 5, tx + 1.5, ty + 5, 0.5);
    }
    o += stipple(0, 50, 18, 8, 7, r);
  } else {
    o += P(`M-30 44 C-16 50 16 50 30 44`, 2.8);
    o += P(`M-27 46 C-15 49 15 49 27 46`, 0.8);
  }

  for (const s of [-1, 1]) {
    for (let w = 0; w < ri(r, 6, 12); w++) {
      const wa = (s > 0 ? -0.2 : Math.PI - 0.2) + rr(r, -0.3, 0.3);
      const wl = rr(r, 12, 30);
      const wx = s * 26 + Math.cos(wa) * rr(r, 2, 6);
      const wy = 40 + Math.sin(wa) * rr(r, 2, 6);
      o += P(`M${wx} ${wy} C${wx + Math.cos(wa) * wl * 0.3} ${wy + Math.sin(wa) * wl * 0.3 - 2} ${wx + Math.cos(wa) * wl * 0.7} ${wy + Math.sin(wa) * wl * 0.7 - 1} ${wx + Math.cos(wa) * wl} ${wy + Math.sin(wa) * wl}`, 0.4);
      o += dot(wx + Math.cos(wa) * wl, wy + Math.sin(wa) * wl, 0.6);
    }
  }

  for (let w = 0; w < ri(r, 3, 6); w++) {
    const wy = -hh * 0.82 + w * 7;
    const ww = hw * 0.28 + w * 2.5;
    o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.5);
    o += P(`M${-ww + 2} ${wy + 1} Q0 ${wy + rr(r, 1, 3)} ${ww - 2} ${wy + 1}`, 0.25);
  }

  o += P(`M${-hw * 0.12} ${-hh * 0.68} Q0 ${-hh * 0.63} ${hw * 0.12} ${-hh * 0.68}`, 0.4);

  o += contourHatch(0, -hh * 0.35, hw * 1.1, hh * 0.35, 7, ri(r, 4, 7), r, 0.25);

  for (let j = 0; j < ri(r, 6, 12); j++) {
    const jx = rr(r, -hw * 0.45, hw * 0.45);
    const jy = rr(r, hh * 0.35, hh * 0.65);
    const jw = rr(r, 7, 16);
    const jh = rr(r, 2, 6);
    o += P(`M${jx - jw} ${jy} C${jx - jw * 0.3} ${jy - jh * 0.4} ${jx + jw * 0.3} ${jy - jh * 0.4} ${jx + jw} ${jy}`, 0.35);
  }
...
function pitbullBody(cx: number, cy: number, sc: number, pose: number, facing: number, r: () => number): string {
  ...
  if (pose === 0) {
    o += P(`M-90 5 C-112 -18 -118 -55 -98 -82 C-78 -102 -28 -118 0 -120 C28 -118 78 -102 98 -82 C118 -55 112 -18 90 5 C80 32 68 72 52 105 L-52 105 C-68 72 -80 32 -90 5Z`, 3);
    o += P(`M-88 7 C-110 -16 -116 -53 -96 -80 C-76 -100 -26 -116 0 -118 C26 -116 76 -100 96 -80 C116 -53 110 -16 88 7 C78 30 66 70 50 103 L-50 103 C-66 70 -78 30 -88 7Z`, 0.8);
    o += furDirection(0, -28, 175, 135, 140, -1.4, r, 0.35);
    o += muscleContour(0, -18, 160, 100, 8, r, 0.5);
    o += contourHatch(0, -45, 145, 65, 14, ri(r, 5, 9), r, 0.25);
    for (let w = 0; w < ri(r, 4, 8); w++) {
      const wy = -85 + w * 9;
      const ww = 55 + w * 5;
      o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 6)} ${ww} ${wy}`, 0.4);
      o += P(`M${-ww + 2} ${wy + 1} Q0 ${wy + rr(r, 1, 3)} ${ww - 2} ${wy + 1}`, 0.2);
    }
    o += P(`M-62 100 C-72 115 -68 135 -58 145 C-48 152 -36 152 -32 145 L-32 128 C-40 123 -48 118 -53 108 C-56 102 -60 100 -62 100`, 2.5);
    o += P(`M62 100 C72 115 68 135 58 145 C48 152 36 152 32 145 L32 128 C40 123 48 118 53 108 C56 102 60 100 62 100`, 2.5);
    o += P(`M-60 102 C-70 117 -66 133 -56 143`, 0.7);
    o += P(`M60 102 C70 117 66 133 56 143`, 0.7);
    o += furDirection(-53, 105, 22, 38, 16, 1.57, r, 0.25);
    o += furDirection(53, 105, 22, 38, 16, 1.57, r, 0.25);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 42, 152, 0.82, 0, r);
    }
    o += P(`M-50 105 C-50 109 -40 112 -28 112 C-13 112 13 112 28 112 C40 112 50 109 50 105`, 1.3);
    o += P(`M-46 107 C-46 110 -36 113 -26 113 C-11 113 11 113 26 113 C36 113 46 110 46 107`, 0.5);
  } else if (pose === 1) {
    o += P(`M-100 10 C-125 -15 -122 -52 -98 -78 C-72 -98 -22 -112 0 -114 C22 -112 72 -98 98 -78 C122 -52 125 -15 100 10 C88 38 72 68 55 95 L-55 95 C-72 68 -88 38 -100 10Z`, 3);
    o += furDirection(0, -20, 190, 105, 150, -1.4, r, 0.35);
    o += muscleContour(0, -8, 170, 80, 7, r, 0.5);
    o += contourHatch(0, -28, 155, 50, 12, ri(r, 5, 9), r, 0.25);
    for (let w = 0; w < ri(r, 4, 8); w++) {
      const wy = -78 + w * 8;
      const ww = 58 + w * 5;
      o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.4);
    }
    o += P(`M-62 90 C-74 105 -68 128 -55 142 C-44 152 -30 152 -26 142 L-26 125 C-35 120 -44 112 -50 100`, 2.5);
    o += P(`M62 90 C74 105 68 128 55 142 C44 152 30 152 26 142 L26 125 C35 120 44 112 50 100`, 2.5);
    o += P(`M-60 92 C-72 107 -66 126 -53 140`, 0.7);
    o += P(`M60 92 C72 107 66 126 53 140`, 0.7);
    o += furDirection(-52, 98, 20, 35, 14, 1.57, r, 0.25);
    o += furDirection(52, 98, 20, 35, 14, 1.57, r, 0.25);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 38, 148, 0.8, 0, r);
    }
    o += P(`M-68 85 C-68 88 -55 90 -40 90 C-20 90 20 90 40 90 C55 90 68 88 68 85`, 1.3);
    o += P(`M-58 95 C-80 105 -100 98 -115 108`, 2.5);
    o += P(`M58 95 C68 105 78 112 82 125 C84 132 78 136 70 132 C64 128 60 120 58 112`, 2.5);
    o += P(`M82 125 C86 130 80 135 72 132`, 0.8);
    o += furDirection(70, 110, 16, 16, 10, -0.4, r, 0.25);
  } else if (pose === 2) {
    o += P(`M-85 -12 C-108 -38 -102 -72 -78 -95 C-52 -112 -18 -122 0 -124 C18 -122 52 -112 78 -95 C102 -72 108 -38 85 -12 C74 15 60 52 45 80 L-45 80 C-60 52 -74 15 -85 -12Z`, 3);
    o += furDirection(0, -22, 165, 100, 135, -1.4, r, 0.35);
    o += muscleContour(0, -10, 150, 85, 7, r, 0.5);
    o += contourHatch(0, -32, 130, 50, 11, ri(r, 5, 8), r, 0.25);
    for (let w = 0; w < ri(r, 3, 7); w++) {
      const wy = -82 + w * 8;
      const ww = 50 + w * 5;
      o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.4);
    }
    o += P(`M-52 75 C-62 88 -58 108 -48 120 C-40 128 -30 128 -26 120 L-26 108 C-33 104 -40 98 -45 88`, 2.5);
    o += P(`M52 75 C62 88 58 108 48 120 C40 128 30 128 26 120 L26 108 C33 104 40 98 45 88`, 2.5);
    o += P(`M-50 77 C-60 90 -56 106 -46 118`, 0.7);
    o += P(`M50 77 C60 90 56 106 46 118`, 0.7);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 35, 126, 0.75, 0, r);
    }
    o += P(`M-45 80 C-55 90 -50 108 -42 118 L-38 108`, 2.5);
    o += P(`M45 80 C55 90 50 108 42 118 L38 108`, 2.5);
    o += P(`M-55 60 C-75 48 -92 30 -102 40 C-108 48 -100 60 -85 62 C-72 64 -62 62 -55 60`, 3);
    o += P(`M55 60 C75 48 92 30 102 40 C108 48 100 60 85 62 C72 64 62 62 55 60`, 3);
    o += P(`M-90 42 C-87 36 -83 39 -85 44`, 0.8);
    o += P(`M90 42 C87 36 83 39 85 44`, 0.8);
    o += furDirection(-70, 48, 20, 18, 12, -0.5, r, 0.25);
    o += furDirection(70, 48, 20, 18, 12, -2.6, r, 0.25);
  } else if (pose === 3) {
    o += P(`M-95 0 C-118 -22 -115 -58 -90 -82 C-62 -102 -15 -115 0 -116 C15 -115 62 -102 90 -82 C115 -58 118 -22 95 0 C84 28 70 62 55 90 L-55 90 C-70 62 -84 28 -95 0Z`, 3);
    o += furDirection(0, -18, 185, 100, 145, -1.4, r, 0.35);
    o += muscleContour(0, -8, 165, 80, 7, r, 0.5);
    o += contourHatch(0, -25, 150, 48, 12, ri(r, 5, 8), r, 0.25);
    for (let w = 0; w < ri(r, 4, 7); w++) {
      const wy = -80 + w * 8;
      const ww = 55 + w * 5;
      o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.4);
    }
    o += P(`M-58 85 C-68 100 -62 125 -50 138 C-40 148 -28 148 -24 138 L-24 122 C-33 117 -42 110 -48 98`, 2.5);
    o += P(`M58 85 C68 100 62 125 50 138 C40 148 28 148 24 138 L24 122 C33 117 42 110 48 98`, 2.5);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 40, 144, 0.78, 0, r);
    }
    o += P(`M42 42 C72 22 105 -8 132 -2 C152 4 148 28 125 38 C105 45 75 42 55 40`, 3.5);
    o += P(`M42 42 C68 24 98 -2 128 2`, 1.2);
    o += furDirection(85, 22, 42, 28, 18, 0.5, r, 0.25);
    o += P(`M122 0 C120 -5 117 -2 119 3`, 0.7);
  } else if (pose === 4) {
    o += P(`M-80 18 C-102 -8 -98 -42 -75 -65 C-50 -82 -15 -92 0 -94 C15 -92 50 -82 75 -65 C98 -42 102 -8 80 18 C68 42 55 72 42 100 L-42 100 C-55 72 -68 42 -80 18Z`, 3);
    o += furDirection(0, -5, 160, 90, 130, -1.4, r, 0.35);
    o += muscleContour(0, 2, 145, 75, 6, r, 0.5);
    o += contourHatch(0, -12, 135, 42, 10, ri(r, 4, 7), r, 0.25);
    o += P(`M-48 95 C-55 108 -50 125 -42 135 L-38 125`, 2.5);
    o += P(`M48 95 C55 108 50 125 42 135 L38 125`, 2.5);
    o += P(`M-58 58 C-78 45 -95 28 -105 38 C-112 46 -103 58 -88 60 C-75 62 -65 60 -58 58`, 3);
    o += P(`M58 58 C78 45 95 28 105 38 C112 46 103 58 88 60 C75 62 65 60 58 58`, 3);
    o += P(`M-92 40 C-89 34 -85 37 -87 42`, 0.8);
    o += P(`M92 40 C89 34 85 37 87 42`, 0.8);
    o += furDirection(-72, 48, 20, 16, 12, -0.5, r, 0.25);
    o += furDirection(72, 48, 20, 16, 12, -2.6, r, 0.25);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 35, 138, 0.72, 0, r);
    }
  } else if (pose === 5) {
    o += P(`M-88 -15 C-112 -42 -105 -78 -80 -100 C-55 -118 -18 -128 0 -130 C18 -128 55 -118 80 -100 C105 -78 112 -42 88 -15 C76 12 62 48 48 78 L-48 78 C-62 48 -76 12 -88 -15Z`, 3);
    o += furDirection(0, -28, 170, 110, 140, -1.4, r, 0.35);
    o += muscleContour(0, -15, 155, 90, 7, r, 0.5);
    o += contourHatch(0, -38, 140, 52, 12, ri(r, 5, 8), r, 0.25);
    for (let w = 0; w < ri(r, 3, 6); w++) {
      const wy = -88 + w * 8;
      const ww = 52 + w * 5;
      o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.4);
    }
    o += P(`M-52 73 C-60 86 -55 105 -46 115 L-42 105`, 2.5);
    o += P(`M52 73 C60 86 55 105 46 115 L42 105`, 2.5);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 38, 120, 0.75, 0, r);
    }
    for (const s of [-1, 1]) {
      o += P(`M${s * 48} ${-88} C${s * 54} ${-108} ${s * 46} ${-128} ${s * 38} ${-122} C${s * 32} ${-118} ${s * 34} ${-98} ${s * 44} ${-90}`, 2.5);
      o += P(`M${s * 46} ${-86} C${s * 52} ${-105} ${s * 44} ${-125} ${s * 40} ${-120}`, 0.7);
      o += furDirection(s * 42, -110, 14, 16, 10, s * -1.0, r, 0.25);
    }
    o += P(`M-48 -82 C-58 -105 -48 -130 -38 -125 C-32 -120 -34 -100 -46 -92`, 2.8);
    o += P(`M48 -82 C58 -105 48 -130 38 -125 C32 -120 34 -100 46 -92`, 2.8);
  } else if (pose === 6) {
    o += P(`M-82 12 C-105 -15 -100 -48 -78 -72 C-55 -90 -18 -100 0 -102 C18 -100 55 -90 78 -72 C100 -48 105 -15 82 12 C70 38 58 65 44 92 L-44 92 C-58 65 -70 38 -82 12Z`, 3);
    o += furDirection(0, -12, 165, 95, 135, -1.4, r, 0.35);
    o += muscleContour(0, -2, 150, 78, 7, r, 0.5);
    o += contourHatch(0, -20, 140, 45, 10, ri(r, 5, 8), r, 0.25);
    o += P(`M-50 87 C-58 100 -53 118 -44 128 L-40 118`, 2.5);
    o += P(`M50 87 C58 100 53 118 44 128 L40 118`, 2.5);
    o += P(`M48 32 C68 18 88 -2 108 4 C122 8 120 28 105 35 C88 40 68 38 52 36`, 3.5);
    o += P(`M82 0 C80 -6 76 -3 78 2`, 0.8);
    o += P(`M-44 92 C-40 98 -34 100 -28 100 C-14 100 14 100 28 100 C34 100 40 98 44 92`, 1.3);
    o += furDirection(68, 22, 30, 22, 14, 0.2, r, 0.25);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 36, 130, 0.72, 0, r);
    }
  } else {
    o += P(`M-82 8 C-105 -18 -100 -50 -78 -75 C-55 -94 -18 -105 0 -106 C18 -105 55 -94 78 -75 C100 -50 105 -18 82 8 C70 35 58 65 44 92 L-44 92 C-58 65 -70 35 -82 8Z`, 3);
    o += furDirection(0, -12, 165, 95, 135, -1.4, r, 0.35);
    o += muscleContour(0, -2, 150, 78, 7, r, 0.5);
    o += contourHatch(0, -18, 140, 45, 10, ri(r, 5, 8), r, 0.25);
    for (let w = 0; w < ri(r, 3, 6); w++) {
      const wy = -75 + w * 8;
      const ww = 50 + w * 5;
      o += P(`M${-ww} ${wy} Q0 ${wy + rr(r, 2, 5)} ${ww} ${wy}`, 0.4);
    }
    o += P(`M-48 87 C-55 100 -50 118 -42 128 L-38 118`, 2.5);
    o += P(`M48 87 C55 100 50 118 42 128 L38 118`, 2.5);
    o += P(`M-58 55 C-78 42 -95 25 -105 35 C-112 43 -103 56 -88 58 C-75 60 -65 58 -58 55`, 3);
    o += P(`M58 55 C78 42 95 25 105 35 C112 43 103 56 88 58 C75 60 65 58 58 55`, 3);
    o += P(`M-92 38 C-89 32 -85 35 -87 40`, 0.8);
    o += P(`M92 38 C89 32 85 35 87 40`, 0.8);
    o += furDirection(-72, 45, 20, 16, 12, -0.5, r, 0.25);
    o += furDirection(72, 45, 20, 16, 12, -2.6, r, 0.25);
    for (const s of [-1, 1]) {
      o += detailedPaw(s * 36, 130, 0.72, 0, r);
    }
  }
...
function pitbullBackground(r: () => number): string {
  let o = "";
  const groundY = rr(r, 870, 940);

  o += P(`M0 ${groundY} C150 ${groundY - 6} 300 ${groundY + 8} 512 ${groundY - 4} C700 ${groundY + 6} 870 ${groundY - 8} 1024 ${groundY}`, 1.5);
  o += P(`M0 ${groundY + 3} C150 ${groundY - 3} 300 ${groundY + 11} 512 ${groundY - 1} C700 ${groundY + 9} 870 ${groundY - 5} 1024 ${groundY + 3}`, 0.5);

  for (let g = 0; g < ri(r, 8, 18); g++) {
    const gx = rr(r, 30, 994);
    const gy = groundY + rr(r, -5, 5);
    const blades = ri(r, 2, 5);
    for (let b = 0; b < blades; b++) {
      const bh = rr(r, 10, 35);
      const bend = rr(r, -15, 15);
      o += P(`M${gx + b * 3} ${gy} C${gx + b * 3 + bend * 0.3} ${gy - bh * 0.4} ${gx + b * 3 + bend * 0.7} ${gy - bh * 0.8} ${gx + b * 3 + bend} ${gy - bh}`, 0.6);
    }
  }

  const annotCount = ri(r, 2, 5);
  for (let a = 0; a < annotCount; a++) {
    const ax = rr(r, 60, 960);
    const ay = rr(r, 40, 200);
    const lineLen = rr(r, 30, 80);
    const lineAngle = rr(r, -0.8, 0.8);
    const endX = ax + Math.cos(lineAngle) * lineLen;
    const endY = ay + Math.sin(lineAngle) * lineLen;
    o += L(ax, ay, endX, endY, 0.4);
    o += dot(ax, ay, 1.2);
  }

  const corner = ri(r, 0, 4);
  if (corner === 0) {
    o += P(`M30 30 L80 30 L80 35 L35 35 L35 80 L30 80 Z`, 0.8);
    o += P(`M994 30 L944 30 L944 35 L989 35 L989 80 L994 80 Z`, 0.8);
  } else if (corner === 1) {
    o += C2(50, 50, 20, 0.6);
    o += C2(50, 50, 15, 0.3);
    o += C2(974, 50, 20, 0.6);
...
function pitbullDetails(cx: number, cy: number, sc: number, r: () => number): string {
  let o = `<g transform="translate(${cx},${cy}) scale(${sc})">`;
  for (let f = 0; f < ri(r, 4, 9); f++) {
    const fy = rr(r, -65, -12);
    const fw = rr(r, 18, 45);
    o += P(`M${-fw} ${fy} C${-fw * 0.3} ${fy + rr(r, 2, 5)} ${fw * 0.3} ${fy + rr(r, 2, 5)} ${fw} ${fy}`, 0.5);
    o += P(`M${-fw * 0.8} ${fy + 1} C${-fw * 0.2} ${fy + rr(r, 2, 5)} ${fw * 0.2} ${fy + rr(r, 2, 5)} ${fw * 0.8} ${fy + 1}`, 0.25);
  }
  for (let w = 0; w < ri(r, 2, 6); w++) {
    const wy = rr(r, -45, 22);
    const wx = rr(r, -30, 30);
    const wl = rr(r, 4, 12);
    const wa = rr(r, -0.4, 0.4);
    o += P(`M${wx} ${wy} C${wx + Math.cos(wa) * wl * 0.3} ${wy + Math.sin(wa) * wl * 0.3} ${wx + Math.cos(wa) * wl * 0.7} ${wy + Math.sin(wa) * wl * 0.7} ${wx + Math.cos(wa) * wl} ${wy + Math.sin(wa) * wl}`, 0.35);
    o += dot(wx + Math.cos(wa) * wl, wy + Math.sin(wa) * wl, 0.4);
  }
  o += `</g>`;
  return o;
}
...
function pitbullPawprints(r: () => number): string {
  let o = "";
  const count = ri(r, 1, 4);
  for (let i = 0; i < count; i++) {
    const px = rr(r, 100, 924);
    const py = rr(r, 850, 970);
    const ps = rr(r, 6, 11);
    const pa = rr(r, -15, 15);
    o += `<g transform="translate(${px},${py}) rotate(${pa})">`;
    o += E(0, 0, ps * 0.55, ps * 0.75, 1.2);
    for (const s of [-1, 1]) {
      o += E(s * ps * 0.35, -ps * 0.55, ps * 0.18, ps * 0.24, 0.8);
      o += E(s * ps * 0.12, -ps * 0.78, ps * 0.16, ps * 0.2, 0.8);
    }
    o += E(0, -ps * 0.82, ps * 0.14, ps * 0.18, 0.8);
    o += `</g>`;
  }
  return o;
}
...
function generatePitbulls(r: () => number): string {
  ...
  const cx = C + rr(r, -50, 50);
  const cy = C + rr(r, -25, 25);
  const sc = rr(r, 0.95, 1.4);
  ...
  const headOffsetY = -68 * sc;
  const headOffsetX = pose === 3 ? 22 * sc * facing : pose === 4 ? 18 * sc * facing : 0;
  o += pitbullHead(cx + headOffsetX, cy + headOffsetY, sc * 0.88, facing, expression, earType, r);

  if (hasCollar) {
    o += pitbullCollar(cx, cy - 8 * sc, sc, r);
  }

  if (hasAccessory) {
    o += pitbullAccessories(cx + headOffsetX, cy + headOffsetY, sc * 0.88, accType, r);
  }
